...
関連なしと判断した場合は `"off_topic": true` を含む JSON を返してください（title_ja, summary_ja 等は空文字でよい）。
...

{
  ...
  "summary_ja": "元記事の情報をできる限り100%に近い形で日本語化する。元記事の文章をそのまま使うのではなく、内容・事実・発言・背景・文脈を全て自分の言葉で日本語に置き換える。省略・要約はしない。ヒップホップファンが元記事を読まなくても同等の情報が得られるレベルを目指す。段落ごとに改行コード(\\n)を入れ、アーティスト名は初出時のみ『カタカナ（英語名）』を併記する（詳細は下記ルール参照）。",
  "category": "カテゴリ名",
...
1. **段落改行**: 話題の区切りごとに改行コード(\\n)を入れて段落分けする。
   ...
   長い文章を一塊で返さず、必ず複数段落に分けること。
2. **アーティスト名の英語併記**:
   - summary_ja 内で各アーティスト名が**初出の1回目のみ**、
     カタカナ表記の直後に半角括弧で英語名を併記する。
     例: 「ケンドリック・ラマー（Kendrick Lamar）が新EPを発表」
     例: 「ドレイク（Drake）とフューチャー（Future）のコラボ曲」
   - 同じアーティストが2回目以降に登場する場合は**カタカナ表記のみ**で書く（括弧併記は禁止）。
   - 元々英語表記が定着している名前（YG, 6ix9ine, Ty Dolla $ign, JID など、
     カタカナ化すると識別性が下がるもの）は**英語のまま**使い、括弧併記は不要。
   - **title_ja には英語併記を入れない**（文字数節約のため）。

## 注意
- アーティスト名・地名・ブランド名はカタカナ表記を優先する（summary_ja / title_ja 内のみ。ただし summary_ja の初出時は上記ルールに従い英語併記する）
- 日本語として不自然な直訳は避け、意訳を心がける
- 渡された本文に含まれていない情報について「不明」「情報は限られている」「詳細はない」等の推測・補足は一切書かない。渡されたテキストの範囲内のみを日本語化すること
- 「この記事はヒップホップとは関連がない」等の編集者コメントは summary_ja に書かない（off_topic フラグで制御するため）
